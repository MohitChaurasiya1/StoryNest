...
import NotificationBell from "../ParentModule/NotificationBell"; // Reusing Parent NotificationBell for now, might need a Teacher-specific one later
...
function TeacherNavbar({
  ...
  const toggleTheme = () => {
    ...
    if (nextDark) {
      ...
      localStorage.setItem("storynest-teacher-theme", "dark");
    } else {
      document.documentElement.classList.remove("dark");
      localStorage.setItem("storynest-teacher-theme", "light");
    }
  };
    ...
    <header className="sticky top-0 z-30 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 transition-colors duration-200">
      <div className="flex h-20 items-center justify-between px-6">
        ...
        <div className="flex items-center gap-4">
          <Link
            to="/teacher/search"
            className="hidden lg:flex items-center gap-3 rounded-xl border border-slate-200 dark:border-slate-700 px-4 py-2 w-80 text-black/70 dark:text-white/80 hover:border-blue-500 transition"
          >
            <FaSearch />
            <span className="text-sm">Search classrooms, students...</span>
          </Link>
